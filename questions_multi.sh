...
\n\nIf you do not have one from your provider leave the Wireguard default below.
\n\nPrivate IP address:" \
		10.0.0.4 StratumInternalIP is_valid_ipv4
fi
...
\n\nStratum Server User Name:" \
		yiimpadmin StratumUser is_valid_username
fi
...
case $response in
	0)
		...
		write_conf_file -m 600 "$STORAGE_ROOT/yiimp/.yiimp.conf" \
			...
			WebUser StratumUser DaemonUser \
	...
	1)
		...
		unset UsingSubDomain DomainName StratumURL InstallSSL SupportEmail DBInternalIP \
			WebInternalIP WebUser WebPass StratumInternalIP StratumUser StratumPass \
